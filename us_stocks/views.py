...
def update_finance(request):
    ...
    for market in markets:
        print 'process market ' + market
        f = open('D:/1IFEELGOOD/' + market + '.txt')
        for l in f:
            ...
            selectOptions = [x for x in selectOptions if '(' not in x]
            print selectOptions
            # print a
                ...
                for item in dataList:
                    ...
                    if  'quarter end date' in item :
                        ...
                        for key,value in values.iteritems():
                            obj = Finance()
                            obj.surrogate_key = key + '_' + item + '_' + market + '_' + symbol
                            obj.symbol = market + '_' + symbol
                            obj.date = int(key)
                            obj.time_type    = 'S'
                            obj.name = item
                            if value is None:
