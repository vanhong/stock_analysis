...
def venue_lookup(request):
    ...
    for venue in venues:
        venue_dict = {
            'id': venue.symbol, 'label': venue.name, 'value': venue.name}
...
def analysis(request, template_name, drawTool):
    ...
    return render_to_response(
        'analysis/' + template_name, {'drawTool': drawTool, 'url': url},
...
def set_stockid(request):
    if 'q' in request.GET:
        ...
        if StockId.objects.filter(symbol=stockid):
            ...
            return render_to_response(
                'analysis/index.html', {"stock_id": name},
                context_instance=RequestContext(request))
        else:
...
>>>>>>> 36ca4dab80fec9c073cb512c1c8ed425cf9068e9
...
def get_profitability_table(request):
    ...
    bodys = []
    if StockId.objects.filter(symbol=symbol):
        season_profitabilitys = SeasonFinancialRatio.objects.filter(
            ...
            for profitability in season_profitabilitys:
                if profitability.gross_profit_margin is not None and profitability.operating_profit_margin  is not None and\
                   profitability.net_before_tax_profit_margin is not None and profitability.net_after_tax_profit_margin is not None:
                    ...
                    if time_type == 'season':
                        item.append(
                            str(profitability.year) + 'Q' + str(profitability.season))
                    elif time_type == 'year':
...
def get_month_revenue_table(request):
    ...
    bodys = []
    if StockId.objects.filter(symbol=symbol):
        month_revenues = MonthRevenue.objects.filter(
            symbol=symbol).order_by('-date')
        if month_revenues:
            for revenue in month_revenues:
                if revenue.month:
                    if revenue.revenue is not None:
                        item = []
                        item.append(
                            str(revenue.year) + r'/' + str(revenue.month))
...
def get_season_revenue_table(request):
    ...
    bodys = []
    if StockId.objects.filter(symbol=symbol):
        season_revenues = SeasonRevenue.objects.filter(
            symbol=symbol).order_by('-date')
        if season_revenues:
...
def get_season_profit_table(request):
    ...
    bodys = []
    if StockId.objects.filter(symbol=symbol):
        season_revenues = SeasonProfit.objects.filter(
            symbol=symbol).order_by('-date')
        if season_revenues:
            for revenue in season_revenues:
    ...
    if StockId.objects.filter(symbol=symbol):
        ...
        for revenue in season_revenues:
            if revenue.revenue is not None and revenue.year_growth_rate is not None:
                ...
                xAxis_categories.append(
                    str(revenue.year) + 'Q' + str(revenue.season))
        if len(revenue_data) > 24:
            dataNum = 24
        else:
            dataNum = len(revenue_data)
    data = {
        'revenue': revenue_data, 'growth_rate': growth_rate_data, 'categories': xAxis_categories,
...
def get_season_profit_chart(request):
    ...
    dataNum = 0
    if StockId.objects.filter(symbol=symbol):
        season_profits = SeasonProfit.objects.filter(
            symbol=symbol).order_by('date')
        profit_data = []
        growth_rate_data = []
        xAxis_categories = []
        for profit in season_profits:
            if profit.profit is not None and profit.year_growth_rate is not None:
                ...
                growth_rate_data.append(float_year_growth_rate)
                xAxis_categories.append(
                    str(profit.year) + 'Q' + str(profit.season))
        if len(profit_data) > 24:
            dataNum = 24
        else:
            dataNum = len(profit_data)
    data = {
        'revenue': profit_data, 'growth_rate': growth_rate_data, 'categories': xAxis_categories,
    ...
    if StockId.objects.filter(symbol=symbol):
        ...
        for revenue in month_revenues:
            if revenue.revenue is not None and revenue.year_growth_rate is not None:
                ...
                xAxis_categories.append(
                    str(revenue.year) + '/' + str(revenue.month).zfill(2))
        if len(revenue_data) > 24:
            dataNum = 24
        else:
            dataNum = len(revenue_data)
...
def get_dividend_chart(request):
    ...
    if StockId.objects.filter(symbol=symbol):
        dividends = Dividend.objects.filter(
            symbol=symbol).order_by('-year')[0:10]
        xAxis_categories = []
        cash_dividends = []
        stock_dividends = []
        for dividend in dividends:
            if dividend.cash_dividends is not None and dividend.stock_dividends is not None:
                ...
                stock_dividends.append(
                    round(float(dividend.stock_dividends), 2))
    data = {'categories': xAxis_categories[::-1], 'cash_dividends': cash_dividends[
        ::-1], 'stock_dividends': stock_dividends[::-1]}
    return HttpResponse(json.dumps(data), content_type="application/json")
...
def get_performance_per_share_chart(request):
    ...
    if time_type == 'year':
        ...
        ratio_model = SeasonFinancialRatio
    if StockId.objects.filter(symbol=symbol):
        season_financial_ratios = ratio_model.objects.filter(
            symbol=symbol).order_by('date')
        xAxis_categories = []
        ...
        for ratio in season_financial_ratios:
            if ratio.net_before_tax_profit_per_share is not None and ratio.net_after_tax_profit_per_share is not None:
                if time_type == 'year':
                    ...
                    xAxis_categories.append(
                        str(ratio.year) + "Q" + str(ratio.season))
                net_before_tax_profit_per_shares.append(
                    float(ratio.net_before_tax_profit_per_share))
                net_after_tax_profit_per_shares.append(
                    float(ratio.net_after_tax_profit_per_share))
    names = [r'稅前每股盈餘', r'稅後每股盈餘']
    datas = [net_before_tax_profit_per_shares, net_after_tax_profit_per_shares]
    yUnit = '元'
    data = {'categories': xAxis_categories,
            'names': names, 'datas': datas, 'yUnit': yUnit}
    return HttpResponse(json.dumps(data), content_type="application/json")
            ...
            if ratio.account_receivable_turnover_ratio is not None and ratio.inventory_turnover_ratio is not None and\
               ratio.fixed_assets_turnover_ratio is not None and ratio.assets_turnover_ratio is not None:
                if time_type == 'year':
                    ...
                    xAxis_categories.append(
                        str(ratio.year) + "Q" + str(ratio.season))
                account_receivable_turnover_ratios.append(
                    float(ratio.account_receivable_turnover_ratio))
                inventory_turnover_ratios.append(
                    float(ratio.inventory_turnover_ratio))
                fixed_assets_turnover_ratios.append(
                    float(ratio.fixed_assets_turnover_ratio))
                assets_turnover_ratios.append(
                    float(ratio.assets_turnover_ratio))
...
def get_profitability_chart(request):
    ...
    symbol = get_symbol(request)
    if StockId.objects.filter(symbol=symbol):
        profitabilitys = ratio_model.objects.filter(
            symbol=symbol).order_by('date')
        xAxis_categories = []
            ...
            if profitability.gross_profit_margin is not None and profitability.operating_profit_margin is not None and \
               profitability.net_before_tax_profit_margin is not None and profitability.net_after_tax_profit_margin is not None:
                ...
                gross_profit_margins.append(
                    float(profitability.gross_profit_margin))
                operating_profit_margins.append(
                    float(profitability.operating_profit_margin))
                net_before_tax_profit_margins.append(
                    float(profitability.net_before_tax_profit_margin))
                net_after_tax_profit_margins.append(
                    float(profitability.net_after_tax_profit_margin))
    names = [r'毛利率', r'營益率', r'稅前淨利率', r'稅後淨利率']
    datas = [
        gross_profit_margins, operating_profit_margins, net_before_tax_profit_margins,
        net_after_tax_profit_margins]
    yUnit = '%'
    data = {'categories': xAxis_categories,
            'names': names, 'datas': datas, 'yUnit': yUnit}
